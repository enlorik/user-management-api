...
**Why the rate limiter is a servlet filter at @Order(1)** — The filter runs before any controller code or business logic, rejecting abusive IPs before requests reach the service and repository layers. One caveat: Spring Boot's Spring Security filter defaults to order −100, which means Security runs before @Order(1) filters — so the rate limiter sits between the security layer and the application layer, not in front of Security itself.
